> 此目录是迁移前的历史示例，以下 README 原文保留。新主线见根目录 README.md。通过根目录 `npm run serve` 后访问 `/legacy/index.html`；旧构建用 `npm run legacy:build`，旧物理测试用 `npm run legacy:verify:integrator -- --edge` 等命令。此处单位 `2M=1` 与新主线不同。
...
npm run verify:termination -- --edge # 终止状态、步数耗尽与背景泄漏检查
npm run verify:absorption -- --edge  # 路程吸收、动量归一化与步长收敛检查
```
...
- 光线只有越过远区边界且继续向外传播时才采样背景星空；步数耗尽或数值异常时保留已累计的前景辐射，不添加背景。低画质可能留下尚未完成追踪的暗区
- 体积盘按中点近似计算物质局部参考系路程 $d\ell=(p_\mu u^\mu)\,d\lambda$，灰吸收系数为 $\alpha=0.28\exp[-(y/h)^2]/r_s$，透射率乘 $\exp(-\alpha d\ell)$；体积发射使用同一路程并计入段内自吸收。离散盘面交点仍采用每次穿越一次的经验薄层贡献
- 帧率不足时会自动降低渲染分辨率
